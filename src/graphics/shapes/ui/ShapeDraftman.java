...
public class ShapeDraftman implements ShapeVisitor {
  ...
  public void visitRectangle(SRectangle sRect) {
    ...
    ColorAttributes color =
        (ColorAttributes)sRect.getAttributes().get(ColorAttributes.ID);
    if (color == null)
    ...
    SelectionAttributes selectionAttributes =
        (SelectionAttributes)sRect.getAttributes().get(SelectionAttributes.ID);
    if (selectionAttributes != null && selectionAttributes.isSelected())
      drawSelectionSquares(sRect.getBounds());
  }
  ...
  public void visitCircle(SCircle sCircle) {
    ...
    ColorAttributes color =
        (ColorAttributes)sCircle.getAttributes().get(ColorAttributes.ID);
    if (color == null)
      color = DEFAULTCOLORATTRIBUTES;
    if (color.filled) {
      g.setColor(color.filledColor);
    ...
    SelectionAttributes selectionAttributes =
        (SelectionAttributes)sCircle.getAttributes().get(
            SelectionAttributes.ID);
    if (selectionAttributes != null && selectionAttributes.isSelected())
      drawSelectionSquares(sCircle.getBounds());
  }
  ...
  public void visitText(SText sText) {
    ...
    FontAttributes fontAttributes =
        (FontAttributes)sText.getAttributes().get(FontAttributes.ID);
    if (fontAttributes == null)
      fontAttributes = DEFAULTFONTATTRIBUTES;
    ColorAttributes color =
        (ColorAttributes)sText.getAttributes().get(ColorAttributes.ID);
    if (color == null)
      color = DEFAULTCOLORATTRIBUTES;
    if (color.filled) {
      g.setColor(color.filledColor);
    ...
    SelectionAttributes selectionAttributes =
        (SelectionAttributes)sText.getAttributes().get(SelectionAttributes.ID);
    if (selectionAttributes != null && selectionAttributes.isSelected())
      drawSelectionSquares(sText.getBounds());
  }

  private void drawSelectionSquares(Rectangle bounds) {
    g.setColor(Color.BLACK);
    g.drawRect(bounds.x - SELECTIONRECTSIZE, bounds.y - SELECTIONRECTSIZE,
               SELECTIONRECTSIZE, SELECTIONRECTSIZE);
    g.setColor(Color.BLACK);
    g.drawRect(bounds.x + bounds.width, bounds.y + bounds.height,
               SELECTIONRECTSIZE, SELECTIONRECTSIZE);
  }
}
