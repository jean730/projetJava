...
public class ShapesController extends Controller {
  ...
  public void mousePressed(MouseEvent e) {
    Shape s = getTarget(e);
    if (s == null)
      this.mousePressedLock = true;
    else {
      SelectionAttributes selectionAttributes =
          (SelectionAttributes)s.getAttributes().get("selection");
  ...
  private void unselectAll() {
    this.collection.iterator().forEachRemaining((shape) -> {
      SelectionAttributes selectionAttributes =
          (SelectionAttributes)shape.getAttributes().get("selection");
      if (selectionAttributes != null)
        selectionAttributes.unselect();
    });
  }
