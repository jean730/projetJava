...
public class GameMain extends JFrame {
  ...
  public void newGame() {
    ...
    this.model = new GameModel(
        new Generator("assets/GrassLand/Terrain/Grassland_Terrain_Tileset.png",
                      512, 32, 0)
            .getTileMap(),
        new Point2D.Double(4000, 100), this);
    ...
    this.gview.requestFocusInWindow();
    Thread t = new Thread() {
      public void run() { ((GameModel)model).gameLoop(); }
    };
    t.start();
  }
  ...
  public void loadGame() {
    this.getContentPane().remove(this.gview);
    this.model = new GameModel(new Loader("assets/save").getTileMap(),
                               new Point2D.Double(50, 100), this);
    ((GameModel)model).getAudio().play("background");
    this.buildEnvironnement();
    this.getContentPane().add(this.gview, java.awt.BorderLayout.CENTER);
    this.pack();
