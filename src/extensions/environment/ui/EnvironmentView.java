...
public class EnvironmentView extends View {
  ...
  public void paintComponent(Graphics g) {
    ...
    this.camera.setTransform(
        -player.getLoc().x * zoom + getWidth() / 2,
        -player.getLoc().y * zoom + (1.0 / 3.0) * getWidth(), zoom, 0);
    ...
    cameraGraphics.fillRect(player.getLoc().x - 240, 0, 480,
                            (int)(getHeight() * zoom));
    g.setColor(new Color(30, 40, 80));
    g.fillRect(0, 0, getWidth(), getHeight());
    Shape shape = (Shape)this.getModel();
    shape.accept(draftman);
    cameraGraphics.drawImage(this.pixieLight, pixie.getLoc().x - 650 + 4,
                             pixie.getLoc().y - 650 + 4, null);
    g.drawImage(this.camera.getImage(), 0, 0, null);
    ((GameModel)shape).getScoreShape().accept(new EnvironmentDraftman(g));
    repaint();
  }
}
