...
public class Player extends Entity {
  ...
  public Player(Point2D.Double loc, GameModel gameModel) {
    ...
    sprite.registerAnimation("idle",
                             new Animation(16, 16, 128, 256, 0, 4, 140));
    sprite.registerAnimation("left",
                             new Animation(16, 16, 128, 256, 8, 4, 140));
    sprite.registerAnimation("right",
                             new Animation(16, 16, 128, 256, 16, 4, 140));
  ...
  public void applyPhysics(TileMap tileMap, double dt) {
    ...
    if (this.getLoc().x >=
        tileMap.getTextureMap()[0].length * tileMap.TILEWIDTH * 0.9) {
  ...
  public Point2D.Double nextLoc(double dt) {
    ...
    return new Point2D.Double(doubleLoc.x + this.velocity.x * dt,
                              doubleLoc.y + this.velocity.y * dt);
  }

  public Boolean onGround(TileMap tileMap, Point2D.Double doubleLoc) {
    try {
      for (int i = 1; i < this.getSprite().getWidth(); i++) {
        if (tileMap.getTextureMap()[(
                int)((doubleLoc.y + this.getSprite().getHeight()) /
                     tileMap.TILEWIDTH)][(int)((doubleLoc.x + i) /
                                               tileMap.TILEWIDTH)] >= 0) {
          return true;
        }
      }
  ...
  public Boolean onRoof(TileMap tileMap, Point2D.Double doubleLoc) {
    try {
      for (int i = 1; i < this.getSprite().getWidth(); i++) {
        if (tileMap.getTextureMap()[(int)(doubleLoc.y / tileMap.TILEWIDTH)][(
                int)((doubleLoc.x + i) / tileMap.TILEWIDTH)] >= 0) {
          return true;
        }
      }
      return false;
  ...
  public Boolean onWallLeft(TileMap tileMap, Point2D.Double doubleLoc) {
    try {
      for (int i = 1; i < this.getSprite().getHeight(); i++) {
        if (tileMap
                .getTextureMap()[(int)((doubleLoc.y + i) / tileMap.TILEWIDTH)]
                                [(int)(doubleLoc.x / tileMap.TILEWIDTH)] >= 0) {
          return true;
        }
      }
      return false;
  ...
  public Boolean onWallRight(TileMap tileMap, Point2D.Double doubleLoc) {
    try {
      for (int i = 1; i < this.getSprite().getHeight(); i++) {
        if (tileMap
                .getTextureMap()[(int)((doubleLoc.y + i) / tileMap.TILEWIDTH)][(
                    int)((doubleLoc.x + this.getSprite().getWidth()) /
                         tileMap.TILEWIDTH)] >= 0) {
          return true;
        }
      }
  ...
  public void conditionalJumpFunctionToJumpOnlyLeftRight(TileMap tileMap) {
    if (onWallLeft(tileMap, this.getDoubleLoc()) &&
        !onGround(tileMap, this.getDoubleLoc()) && Jumping == 1) {
      this.jump();
      this.velocity.x += 300;
    }
    if (onWallRight(tileMap, this.getDoubleLoc()) &&
        !onGround(tileMap, this.getDoubleLoc()) && Jumping == 1) {
      this.jump();
      this.velocity.x += -300;
    }
  ...
  public void conditionalJumpFunctionToJumpOnlyOnGround(TileMap tileMap) {
    if (onGround(tileMap, this.getDoubleLoc()))
      this.jump();
  }
  ...
  public void applyColisions() {
    for (Entity e : this.gameModel.getEntities()) {
      ...
      if (!inter.isEmpty() && e != this && e.isColisionable()) {
        ...
        if (inter.height < inter.width && this.getLoc().y < e.getLoc().y)
          e.die();
        else
          this.die();
      }
  ...
  public void walk(double dt) {
    ...
    this.velocity.x =
        this.velocity.x + WALKSTRENGTH * factor * dt * walkingDirection;
    if (Math.abs(this.velocity.x) > MAXWALKSPEED * factor)
      this.velocity.x = MAXWALKSPEED * factor * walkingDirection;
  }
